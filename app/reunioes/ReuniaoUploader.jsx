...
export default function ReuniaoUploader() {
  ...
  function juntar(fileList) {
    ...
    setAviso(ignorados ? `${ignorados} ficheiro(s) ignorado(s) (usa áudio ou vídeo).` : "");
  ...
  async function enviarTodos() {
    ...
    for (let i = 0; i < itens.length; i++) {
      ...
      try {
        ...
        // 3) dispara a transcrição + resumo (modo reunião)
        const resp = await fetch("/api/ingest", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ url: prep.publicUrl, filename: file.name, modo: "reuniao" }),
        });
  ...
  return (
    <div className="list" style={{ maxWidth: 620 }}>
      ...
      >
        <div style={{ fontWeight: 650 }}>🎙️ Arrasta as gravações para aqui</div>
        <div className="hint">Áudio ou vídeo de reuniões · cada uma vira transcrição + resumo</div>
        <input
          ref={inputRef}
          type="file"
          multiple
      ...
      <button className="btn" onClick={enviarTodos} disabled={correr || porEnviar === 0}>
        {correr ? "A enviar…" : porEnviar > 0 ? `Enviar ${porEnviar} reunião${porEnviar === 1 ? "" : "s"}` : feitos > 0 ? "Tudo enviado ✓" : "Enviar"}
      </button>

      {feitos > 0 && !correr && (
        <div className="txt" style={{ fontSize: 13.5 }}>
          {feitos} enviada{feitos === 1 ? "" : "s"} para processar. A transcrição e o resumo aparecem aqui
